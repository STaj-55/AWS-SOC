...
Now let's have some fun with Incident Response and Threat Detection Simulations

### 5. Incident Response and Threat Detection Simulations

For this portion of our lab we will be simulating incidents in our SOC and showcase response actions.

We will be simulating four scenarios for this portion. They are as follows:
1. Unauthorized access attempts (Brute Force SSH Attacks)
2. Suspicious network activity (Unusual VPC traffic)
3. Malicious API activity (AWS API calls from unknown locations)
4. Indicators of compromise (GuardDuty findings for malware or port scanning)

#### 5.1 Simulation 1: Brute Force SSH Attacks

#### 5.1.1 Preparation (CloudWatch)

First we will ensure that SSH auditing is enabled, we can do this by enabling it in the sshd config file
- sudo nano /etc/ssh/sshd_config
- Uncomment / Add in this line: LogLevel VERBOSE
- sudo systemctl restart sshd
- sudo cat /var/log/audit/audit.log | grep 'ssh' (This ensures that we can see SSH in our logs)

We will be attacking our Web EC2 Instance for these simulations. Because of this we need to install a CloudWatch Agent on the EC2 Instance.

Now let's install the CloudWatch Agent
- sudo yum install -y amazon-cloudwatch-agent

After installing the CloudWatch Agent, we will need to edit our Agent Config file to monitor /var/log/secure for any SSH attempts.
- sudo nano /opt/cloudwatch-agent-config.json
...
Now that our config file is updated, let's start the CloudWatch Agent.
...
To have our configuration to take in effect we can restart our agent:
- sudo systemctl restart amazon-cloudwatch-agent

Now we can check the CloudWatch console to see if our new log group has been created, it should look as such:

![aws32](https://github.com/user-attachments/assets/715c98b6-0ee2-4458-affb-4930faddd28f)

Now we need to test that log group works, from my laptop (contains a different IP) I will attempt to SSH 3 times. If this work it should appear in CloudWatch

At the CloudWatch Console, head to Log Insights and select SSH-Attempts as the Selection criteria

Here is the query we will use:
fields @timestamp, @message
| filter @message like "ssh"
| sort @timestamp desc
| stats count() by bin(30s)

![aws33](https://github.com/user-attachments/assets/533cdadd-9642-4f88-8235-823c8bac1f52)

As we can see by the screenshot above, we were able to visualize our query and the orange part of the pie represents our 3 failed SSH attempts.

Now that this works, let's create an Alarm!

Head from the CloudWatch Console > All Alarms > Create Alarm
- Metric Name: IncomingLogEvents
- Log Group Name: SSH-Attempts
- Statistic: Average
- Period: 1 Minute
  
Conditions
- Threshold Type: Static
- Whenever IncomingLogEvents is... : Greater/Equal
- than... : 5
  
Notification
- Alarm state trigger: In Alarm
- Send a noti to SNS topic: Create new topic
- Create a new topic... : SSH-Brute-Force
- Email endpoints that will receive the notification... : your email
  
Name and description
- Alarm Name: BruteForceSSH
- Alarm Description: Brute Force SSH is currently underway!

With our alarm set up, let's simulate a brute force attack!
